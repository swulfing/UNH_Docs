...
$\frac{dX_1}{dt} = k_1  X_1  (1-X_1) [d_1(2X_1 - 1)+ \frac{1}{P_1+c_1}-w_1] + k_1X_2(1-X_2)[d_1(2X_2-1)+\frac{1}{P_1 + c_1} - w_1]$

$\frac{dP_2}{dt} = r_2P_2(1-P_2)-\frac{h_2*P_2(1-X_2)}{P_2 + s_2} + \rho_2 * \frac{P_1 - P_2}{2}$

$\frac{dX_2}{dt} = k_2  X_2  (1-X_2) [d_2(2X_2 - 1)+ \frac{1}{P_2+c_2}-w_2] + k_2X_1(1-X_1)[d_2(2X_1-1)+\frac{1}{P_2 + c_2} - w_2]$
...
```{r R, fig.cap= "R - Net growth/fecundity"}
param_ranges(1, 0, 1)
```

\newpage

```{r s, fig.cap = "S - supply and demand"}
param_ranges(2, 0, 1)
```

\newpage

```{r h, fig.cap = "h - Harvesting efficiency"}
param_ranges(3, 0, 1)
```

\newpage

```{r k, fig.cap = "K - Social learning rate"}
param_ranges(4, 0, 1)
```
