...
Use dispersal term instead of e and i 
Put outside influence within x(1-x)
...
$\frac{dX_1}{dt} =  k_1X_1(1-X_1) [\frac{1}{P_1+c_1} - w_1 + d_1(2X_1 - 1)] + prop_1X_1(1-X_1)(2X_2 - 1)$

$\frac{dX_2}{dt} = k_2X_2(1-X_2)  [\frac{1}{P_2+c_2} - w_2 + d_2(2X_2 - 1)] + prop_2X_2(1-X_2)(2X_1 - 1)$
...
Bauch.Coupled <- function (t, y, params) {
  ...
  dX_1dt <- k_1 * X_1 * (1-X_1) *(1/(P_1 + c_1) - w_1 + d_1*(2*X_1 - 1)) + prop_1 * X_1 * (1-X_1) * (2* X_2 - 1)# + prop_1 * (1 - X_1) * (2* X_2 - 1)^2
  dX_2dt <- k_2 * X_2 * (1-X_2) *(1/(P_2 + c_2) - w_2 + d_2*(2*X_2 - 1)) + prop_2 * X_2 * (1-X_2) * (2* X_1 - 1)# + prop_2 * (1 - X_2) * (2* X_1 - 1)^2
  ## combine results into a single vector
...
paramplane <- function(paramx, paramy, maxx, maxy){
  ...
  params_Default <- data.frame(R = 0.06, s = .8, h = 0.075, k = 0.17, w = 0.35, c = 0.6, d = 0.5, e = .1, i = .1, prop = .1)
...
popplane <- function(param, max){
  ...
  for(i in 1:nrow(paramcombos)){
  params_Default <- data.frame(R = 0.06, s = .8, h = 0.075, k = 0.17, w = 0.35, c = 0.6, d = 0.5, e = 0.1, i = 0.1, prop = .1)
  #Change params to be the
