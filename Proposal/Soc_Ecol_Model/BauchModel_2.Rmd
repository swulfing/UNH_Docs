...
$\frac{dX_1}{dt} =  k_1X_1(prop_1-X_1)X_2(prop_2-X_2)  [\frac{1}{P_1+c_1} - w_1 + d_1(2X_1-prop_1) + d_2(2X_2 -prop_2)]$

$\frac{dP_2}{dt} = r_2P_2(1-P_2)-\frac{h_2*P_2(1-X_2)}{P_2 + s_2} + \rho_2 * \frac{P_1 - P_2}{2}$

$\frac{dX_2}{dt} = k_1X_1(prop_1-X_1)X_2(prop_2-X_2)  [\frac{1}{P_2+c_2} - w_2 + d_1(2X_1-prop_1) + d_2(2X_2 -prop_2)]$


```{r Bauch.Coupled_firsttry, eval = FALSE}
...
```{r roe, fig.cap = "roe - fish diffusion"}
param_ranges(8, 0, 1)
```

```{r paramplane_diffparams}
...
paramplane <- function(paramx, paramy, maxx, maxy){
  ...
  paramcombos <- expand.grid(x_parameter = seq(from = startx, to = maxx, length.out = 10),
                        y_parameter = seq(from = starty, to = maxy, length.out = 10))
  ...
  finalDynamics <- outcomes %>%
    ...
    XParameter >= .25 & YParameter >= .25 ~ "Both",
    XParameter >= .25 & YParameter < .25 ~ "Only X",
    XParameter < .25 & YParameter >= .25 ~ "Only Y",
    XParameter < .25 & YParameter < .25 ~ "Neither"))
  
cols <- c("green", "yellow", "orange", "black")
...
popplane <- function(param, max){
  ...
  finalDynamics <- outcomes %>%
    mutate(Result = case_when(
    Pop1Parameter >= .25 & Pop2Parameter >= .25 ~ "Both",
    Pop1Parameter >= .25 & Pop2Parameter < .25 ~ "Only Pop1",
    Pop1Parameter < .25 & Pop2Parameter >= .25 ~ "Only Pop2",
    Pop1Parameter < .25 & Pop2Parameter < .25 ~ "Neither"))
