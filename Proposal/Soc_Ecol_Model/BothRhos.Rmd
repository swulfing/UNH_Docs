...
```{r Bauch.Coupled, warning = FALSE, eval = TRUE}
...
bottom = richtext_grob(
  text = paste('Time (year)',  " X1=", xstart[3], "X2=", xstart[4])
)
...
fStarts <- c(0.25, 0.5, 0.75)
xStarts <- c(0.10, 0.25, 0.5)
...
for(i in 1:length(xStarts)){
  ...
  for (j in 1:length(xStarts)){
    
    StartParams <- c(F_1 = 0.406, F_2 = 0.406, X_1 = xStarts[i], X_2 = xStarts[j])
